...
CREATE COMPUTE POOL IF NOT EXISTS gpu_3
    MIN_NODES = 1
    MAX_NODES = 1
    INSTANCE_FAMILY = gpu_3;
GRANT ALL ON COMPUTE POOL gpu_3 TO ROLE nac WITH GRANT OPTION;
GRANT USAGE ON WAREHOUSE wh_nac TO ROLE nac WITH GRANT OPTION;

CREATE DATABASE IF NOT EXISTS llama2_consumer;
CREATE SCHEMA IF NOT EXISTS llama2_consumer.data;
USE SCHEMA llama2_consumer.data;
...
GRANT USAGE ON DATABASE llama2_consumer TO ROLE nac WITH GRANT OPTION;
GRANT USAGE ON SCHEMA llama2_consumer.data TO ROLE nac WITH GRANT OPTION;
GRANT ALL ON TABLE llama2_consumer.data.customer_support_transcripts TO ROLE nac WITH GRANT OPTION;
